...
class Repository {
    ...
    constructor(repository, repositoryResolver, pushErrorHandlerRegistry, remoteSourcePublisherRegistry, postCommitCommandsProviderRegistry, branchProtectionProviderRegistry, globalState, logger, telemetryReporter) {
        ...
        this._historyProvider = new historyProvider_1.GitHistoryProvider(this, logger);
        ...
        const actionButton = new actionButton_1.ActionButton(this, this.commitCommandCenter);
    ...
    getConfig(key) {
        return this.run(operation_1.Operation.Config(true), () => this.repository.config('local', key));
    }
    getGlobalConfig(key) {
        return this.run(operation_1.Operation.Config(true), () => this.repository.config('global', key));
    }
    setConfig(key, value) {
        return this.run(operation_1.Operation.Config(false), () => this.repository.config('local', key, value));
    }
    log(options) {
        const showProgress = !options || options.silent !== true;
        return this.run(operation_1.Operation.Log(showProgress), () => this.repository.log(options));
    }
    logFile(uri, options) {
    ...
    diffTrees(treeish1, treeish2) {
        return this.run(operation_1.Operation.Diff, () => this.repository.diffTrees(treeish1, treeish2));
    }
    ...
    async blame2(path) {
        return await this.run(operation_1.Operation.Blame(false), () => this.repository.blame2(path));
    }
    sync(head, rebase) {
    ...
    async show(ref, filePath) {
        return await this.run(operation_1.Operation.Show, async () => {
            ...
            const candidateGuessEncodings = configFiles.get('candidateGuessEncodings');
            try {
