...
async function createModel(context, logger, telemetryReporter, disposables) {
    ...
    disposables.push(cc, new fileSystemProvider_1.GitFileSystemProvider(model), new decorationProvider_1.GitDecorations(model), new blame_1.GitBlameController(model), new timelineProvider_1.GitTimelineProvider(model, cc), new editSessionIdentityProvider_1.GitEditSessionIdentityProvider(model), new terminal_1.TerminalShellExecutionManager(model, logger));
