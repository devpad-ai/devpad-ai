...
class CheckoutItemsProcessor extends RefItemsProcessor {
    constructor(processors, repository, buttons, detached = false) {
        super(processors);
        this.repository = repository;
...
class CommandCenter {
    ...
    async unstageSelectedRanges(changes) {
        const textEditor = vscode_1.window.activeTextEditor;
    ...
    async checkoutRefDetached(repository, historyItem) {
        if (!historyItem) {
            return false;
        }
    ...
    async deleteTag(repository) {
        const tagPicks = async () => {
            ...
            return remoteTags.length === 0 ? [{ label: vscode_1.l10n.t('$(info) This repository has no tags.') }] : remoteTags.map(ref => new TagDeleteItem(ref));
        };
    ...
    async deleteRemoteTag(repository) {
        ...
        const remoteTagPicks = async () => {
            ...
            return remoteTags.length === 0 ? [{ label: vscode_1.l10n.t('$(info) Remote "{0}" has no tags.', remoteName) }] : remoteTags.map(ref => new RemoteTagDeleteItem(ref));
        };
    ...
    async cherryPickRef(repository, historyItem) {
        if (!historyItem) {
            return;
        }
    ...
    async _resolveTimelineOpenCommitCommand(item, uri, options) {
        ...
        const multiDiffSourceUri = (0, uri_1.toGitUri)(vscode_1.Uri.file(repository.root), commit.hash, { scheme: 'git-commit' });
        const resources = [];
        for (const change of changes) {
            resources.push((0, uri_1.toMultiFileDiffEditorUris)(change, commitParentId, commit.hash));
        }
        return {
    ...
    async viewCommit(repository, historyItem1, historyItem2) {
        ...
        if (!historyItem2) {
            ...
            title = vscode_1.l10n.t('All Changes ({0} ↔ {1})', historyItem2.id.substring(0, 8), historyItem1.id.substring(0, 8));
            historyItemParentId = historyItem2.parentIds.length > 0 ? historyItem2.parentIds[0] : `${historyItem2.id}^`;
        }
        const multiDiffSourceUri = (0, uri_1.toGitUri)(vscode_1.Uri.file(repository.root), `${historyItemParentId}..${historyItem1.id}`, { scheme: 'git-commit', });
        await this._viewChanges(repository, historyItem1.id, historyItemParentId, multiDiffSourceUri, title);
    }
    async viewAllChanges(repository, historyItem) {
        ...
        const title = vscode_1.l10n.t('All Changes ({0} ↔ {1})', originalShortRef, modifiedShortRef);
        const multiDiffSourceUri = (0, uri_1.toGitUri)(vscode_1.Uri.file(repository.root), historyItem.id, { scheme: 'git-changes' });
        await this._viewChanges(repository, modifiedShortRef, originalShortRef, multiDiffSourceUri, title);
    }
    async _viewChanges(repository, historyItemId, historyItemParentId, multiDiffSourceUri, title) {
    ...
    async viewStatusBarCommit(repository, historyItemId) {
        ...
        const multiDiffSourceUri = (0, uri_1.toGitUri)(vscode_1.Uri.file(repository.root), `${historyItemParentId}..${historyItemId}`, { scheme: 'git-commit', });
        const changes = await repository.diffBetween(historyItemParentId, historyItemId);
        const resources = changes.map(c => (0, uri_1.toMultiFileDiffEditorUris)(c, historyItemParentId, historyItemId));
        await vscode_1.commands.executeCommand('_workbench.openMultiDiffEditor', { multiDiffSourceUri, title, resources });
    }
...
], CommandCenter.prototype, "checkoutRefDetached", null);
__decorate([
...
], CommandCenter.prototype, "cherryPickRef", null);
__decorate([
...
], CommandCenter.prototype, "viewAllChanges", null);
__decorate([
...
], CommandCenter.prototype, "viewStatusBarCommit", null);
