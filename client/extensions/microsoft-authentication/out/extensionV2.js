...
async function initMicrosoftSovereignCloudAuthProvider(context, uriHandler) {
    ...
    const authProvider = new authProvider_1.MsalAuthProvider(context, new telemetryReporter_1.MicrosoftSovereignCloudAuthenticationTelemetryReporter(context.extension.packageJSON.aiKey), vscode_1.window.createOutputChannel(vscode_1.l10n.t('Microsoft Sovereign Cloud Authentication'), { log: true }), uriHandler, env);
...
async function activate(context, mainTelemetryReporter) {
    ...
    const authProvider = new authProvider_1.MsalAuthProvider(context, mainTelemetryReporter, logger_1.default, uriHandler);
    await authProvider.initialize();
    context.subscriptions.push(vscode_1.authentication.registerAuthenticationProvider('microsoft', 'Microsoft', authProvider, { supportsMultipleAccounts: true }));
